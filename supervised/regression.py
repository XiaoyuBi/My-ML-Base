...
from utils.layers.activation import Sigmoid
...
class LogisticRegression:
    """
    Logistic Regression training and scoring Interface
    """
    ...
    def train(self, X: np.ndarray, y: np.ndarray):
        ...
        for _ in range(self.n_iter):
            ...
            self.training_losses.append(loss)
            # backward
            dL_dy_ = self.bceloss.backward() * self.sigmoid.backward()
            dL_dX = self.linear.backward(dL_dy_)
            # update
            self.linear.step(self.lr, self.momentum)
            self.linear.zero_grad()
    ...
    def forward(self, X: np.ndarray):
        if not hasattr(self, "linear"):
            raise ValueError("Train the logistic regression model first!")
